execute at @s run effect give @n[type=#main:warding_targets_slowed,predicate=!main:wearing_copper_armour,distance=..8] minecraft:slowness 1 1 true
execute at @s run damage @n[type=#main:warding_targets,type=!wither,predicate=!main:wearing_copper_armour,distance=..6] 1 minecraft:out_of_world
